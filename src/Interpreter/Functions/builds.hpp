...
class Builder
{
	...
	std::vector< std::string > m_main_srcs;
...
std::vector< std::string > & Builder::MainSources() { return m_main_srcs; }
...
int Builder::SetBuildType( const std::string & build_type )
{
	...
	if( build_type != "binary" && build_type != "library" && build_type != "test" ) {
